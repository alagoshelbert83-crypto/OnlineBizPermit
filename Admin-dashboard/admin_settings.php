...
// Include database and session setup first
...
try {
    $stmt = $conn->prepare("SELECT name, email FROM users WHERE id = ?");
    $stmt->execute([$current_user_id]);
    $user_info_display = $stmt->fetch(PDO::FETCH_ASSOC);
    $current_user_name = $user_info_display['name'] ?? 'User';
